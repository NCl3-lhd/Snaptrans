...
void SettingsWindow::render(bool &out_need_font_rebuild, std::string &out_current_locale) {
  ...
  if (ImGui::BeginTabBar("SettingsTabBar", ImGuiTabBarFlags_None)) {
    ...
    renderPreferencesTab(out_need_font_rebuild, out_current_locale);
    renderControlTab();
    renderAboutTab();
...
// 子选项卡 1：通用设置
// ==========================================
void SettingsWindow::renderPreferencesTab(bool &out_need_font_rebuild, std::string &out_current_locale) {
